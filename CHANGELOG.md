...
### Save 按钮重设计
- **打勾动画**: 点击保存后按钮内显示进度条（左→右），完成后打勾图标 + 绿色背景
- **fv-claude 不自动重启**: 保存后只显示提示"请打开新终端输入 claude 启动 Claude Code"
- **fv-codex 自动重启**: 保存后自动调用重启 API，用户可直接使用
- **i18n**: 中英文双语支持所有新增文案

### SSE 保活修复
- 恢复 `event: ping` 保活格式，修复 Claude Code 客户端断连重连问题
